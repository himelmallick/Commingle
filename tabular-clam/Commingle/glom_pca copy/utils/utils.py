...
def get_optim(model, args):
	if args['opt'].item() == "adam":
		optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args['lr'].item(), weight_decay=args['reg'].item())
	elif args['opt'].item() == 'sgd':
		optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=args['lr'].item(), momentum=0.9, weight_decay=args['reg'].item())
	else:
