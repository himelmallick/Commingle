...
class CLAM_SB(nn.Module):
	def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=7, n_classes=2,
		...
		self.size_dict = { "small": [30, 100, 16], "big": [207, 100, 50], "tiny": [3000, 1024, 256]} #RL
		size = self.size_dict[size_arg]
		fc = [nn.Linear(size[0], size[1]), nn.Tanh()] # ReLU
		if dropout:
...
class CLAM_MB(CLAM_SB):
	def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=7, n_classes=2,
		instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False, for_harmony=False):
		nn.Module.__init__(self)
		#self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
		self.size_dict = {"small": [315, 100, 30], "big": [207, 100, 30], "tiny": [1024, 512, 256]}
		if for_harmony:
		    self.size_dict = {"small": [315, 100, 30], "big": [207, 100, 30], "tiny": [50,100,16]}  
		size = self.size_dict[size_arg]
		fc = [nn.Linear(size[0], size[1]), nn.Tanh()] # ReLU
		if dropout:
			fc.append(nn.Dropout(0.25))
