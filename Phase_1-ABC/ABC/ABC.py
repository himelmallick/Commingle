# Adapted from https://github.com/reutd/ABC/blob/master/ABC/ABC.py
...
class ABC(keras.Model):
    ...
    def __init__(self, adata, batch_label, type_label, latent_dim=64):
        ...
        if adata.n_vars > 1024:
            adata = scib.preprocessing.hvg_batch(adata, batch_key=batch_label, target_genes=1024, adataOut=True)
    ...
    def batch_correction(self, load_trained=False, checkpoint_filepath=None, data_name='dataset',
                         base_LR=None, subsample_training=None, recon_loss_w=0.8, epochs=100,
        ...
        if not load_trained:
            ...
            if early_stopping:
                # define callbacks
                callbacks.append(EarlyStopping(monitor='classifier_loss', mode='min', patience=50, min_delta=delta))
                save_best_callback = tf.keras.callbacks.ModelCheckpoint(
                    filepath=checkpoint_filepath,
                    save_weights_only=True,
                    monitor='model_loss', # 'classifier_loss',
                    mode='min',
                    verbose=1,
                    save_best_only=True)
